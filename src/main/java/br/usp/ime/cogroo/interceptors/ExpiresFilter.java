 ...
 * >xebia-servlet-extras-1.0.3.jar</a> (<a href=
 * "http://xebia-france.googlecode.com/files/xebia-servlet-extras-1.0.3-sources.jar"
 * >sources</a>) in your web application classpath.</li>
 * <li>Java class : <a href=
 * "http://xebia-france.googlecode.com/svn/web/xebia-servlet-extras/trunk/src/main/java/fr/xebia/servlet/filter/ExpiresFilter.java"
...
public class ExpiresFilter implements Filter {
	...
	protected Date getExpirationDate(HttpServletRequest request,
		...
		if (configuration == null) {
			logger.trace("No Expires configuration found for content-type {}",
					contentType);
			return null;
		}

		if (logger.isTraceEnabled()) {
			logger.trace("Use {} matching '{}' for content-type '{}'",
					new Object[] { configuration, matchingContentType,
							contentType });
		}
		...
		case LAST_MODIFICATION_TIME:
			...
			break;
		default:
			throw new IllegalStateException("Unsupported startingPoint '"
		...
		for (Enumeration<String> names = filterConfig.getInitParameterNames(); names
			...
			try {
				if (name.startsWith(PARAMETER_EXPIRES_BY_TYPE)) {
					...
					logger.warn("Uknown parameter '" + name + "' with value '"
							+ value + "' is ignored !");
				}
			} catch (RuntimeException e) {
				throw new ServletException(
						"Exception processing configuration parameter '" + name
								+ "':'" + value + "'", e);
			}
		}
	...
	protected boolean isEligibleToExpirationHeaderGeneration(
		...
		if (expirationHeaderHasBeenSet) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Request '{}' with response status '{}' content-type '{}', expiration header already defined",
	...
	public void onBeforeWriteResponseBody(HttpServletRequest request,
		...
		if (expirationDate == null) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Request '{}' with response status '{}' content-type '{}', no expiration configured",
						new Object[] { request.getRequestURI(),
								response.getStatus(), response.getContentType() });
			}
		} else {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Request '{}' with response status '{}' content-type '{}', set expiration date {}",
						new Object[] { request.getRequestURI(),
								response.getStatus(),
	...
	protected ExpiresConfiguration parseExpiresConfiguration(String line) {
		...
		try {
			...
			throw new IllegalStateException(
					"Starting point (access|now|modification|a<seconds>|m<seconds>) not found in directive '"
							+ line + "'");
		}
		...
		if ("access".equalsIgnoreCase(currentToken)
			...
			throw new IllegalStateException(
					"Invalid starting point (access|now|modification|a<seconds>|m<seconds>) '"
							+ currentToken + "' in directive '" + line + "'");
		}
		...
		while (currentToken != null) {
			...
			try {
				...
				throw new IllegalStateException("Invalid duration (number) '"
						+ currentToken + "' in directive '" + line + "'");
			}
			...
			try {
				...
				throw new IllegalStateException(
						"Duration unit not found after amount " + amount
								+ " in directive '" + line + "'");
			}
			...
			if ("years".equalsIgnoreCase(currentToken)) {
				...
				throw new IllegalStateException(
						"Invalid duration unit (years|months|weeks|days|hours|minutes|seconds) '"
								+ currentToken + "' in directive '" + line
								+ "'");
			}
