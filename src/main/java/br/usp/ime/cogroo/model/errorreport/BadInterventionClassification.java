...
public enum BadInterventionClassification {
	...
	FALSE_ERROR("falseError"), INAPPROPRIATE_DESCRIPTION(
			"inappropriateDescription"), INAPPROPRIATE_SUGGESTION(
	...
	public static BadInterventionClassification fromValue(String v) {
		for (BadInterventionClassification c : BadInterventionClassification
				.values()) {
			if (c.value.equals(v)) {
				return c;
			}
