...
public class RuleController {
	...
	public void deprecatedRuleList() {
		result.use(Results.status()).movedPermanentlyTo(RuleController.class)
				.ruleList();
	}
	...
	public void ruleList() {
		...
		result.include("headerTitle", "Regras")
				.include(
						"headerDescription",
						"Exibe as regras utilizadas pelo corretor gramatical CoGrOO para identificar erros.")
	...
	public void ruleStatus() {
		...
		if (user != null) {
			if (user.getRole().getCanRefreshStatus()) {
				result.include("refresh", "R").include("user",
						user.getRole().getCanRefreshStatus());
	...
	public void rule(String id) {
		...
		if (!id.equals(ruleID)) {
			result.use(Results.status())
					.movedPermanentlyTo(RuleController.class).rule(ruleID);
			return;
		}
		...
		List<Pair<Pair<String, List<ProcessResult>>, Pair<String, List<ProcessResult>>>> exampleList = new ArrayList<Pair<Pair<String, List<ProcessResult>>, Pair<String, List<ProcessResult>>>>();

		for (Example example : rule.getExamples()) {

			List<ProcessResult> incorrect = cogroo.cachedProcessText(example
					.getIncorrect());
			List<ProcessResult> correct = cogroo.cachedProcessText(example
					.getCorrect());

			String incorrectStr = cogroo.getAnnotatedText(
					example.getIncorrect(), incorrect);
			String correctStr = cogroo.getAnnotatedText(example.getCorrect(),
					correct);

			Pair<String, List<ProcessResult>> incorrectPair = new Pair<String, List<ProcessResult>>(
					incorrectStr, incorrect);
			Pair<String, List<ProcessResult>> correctPair = new Pair<String, List<ProcessResult>>(
					correctStr, correct);

			Pair<Pair<String, List<ProcessResult>>, Pair<String, List<ProcessResult>>> examplePair = new Pair<Pair<String, List<ProcessResult>>, Pair<String, List<ProcessResult>>>(
					incorrectPair, correctPair);
		...
		result.include("rule", rule)
				...
				.include("previousRule",
						rulesLogic.getPreviousRuleID(rule.getId()));
		...
		if (rule.isXMLBased()) {
			...
			result.include("pattern", RuleUtils.getPatternAsHTML(ruleXML))
					.include("replacePattern",
							RuleUtils.getSuggestionsAsHTML(ruleXML))
					.include("active", ruleXML.isActive())
					.include("method", ruleXML.getMethod());
		}

		String title = "Regra " + rule.getId() + ": " + rule.getShortMessage();
