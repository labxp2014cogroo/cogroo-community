...
public class SyntaxTreeController {
	...
	private static File createTree(String data) throws Exception {
		data = data.replace('_', ' ');
		String cookie;
		Pattern stgraph = Pattern.compile("(stgraph.png.*?)\"",
				Pattern.MULTILINE);
		String targetURL = "http://ccsl.ime.usp.br/cogroo/phpsyntaxtree/?";
		String urlParameters = "data=" + URLEncoder.encode(data, ISO)
				+ "&antialias="
				+ URLEncoder.encode("on", ISO)
				// + "&autosub=" + URLEncoder.encode("off", ISO)
				+ "&color=" + URLEncoder.encode("on", ISO) + "&triangles="
				+ URLEncoder.encode("off", ISO) + "&fontsize="
				+ URLEncoder.encode("10", ISO);
