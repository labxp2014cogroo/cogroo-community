...
public class UnsubscribeController {
	...
	public void doUnsubscription(User user, String codeRecover,
			boolean isReceiveTransactionalMail, boolean isReceiveNewsMail) {
		...
		result.include("okMessage",
				"Preferências de e-mail alteradas com sucesso!");
			...
			if (userFromDB != null) {
				if (!userFromDB.getEmailOptOutCode().equals(codeRecover)) {
					...
					validator.add(new ValidationMessage(
							ExceptionMessages.BAD_OPT_OUT_CODE,
							ExceptionMessages.ERROR));
				}
			} else {
