...
public class ErrorReportController {
	...
	public ErrorReportController(LoggedUser loggedUser,
			ErrorEntryLogic errorEntryLogic, Result result,
			Validator validator, ErrorEntryDAO errorEntryDAO,
			CommentDAO commentDAO, SecurityUtil securityUtil,
			CogrooFacade cogrooFacade, AnalyticsManager manager,
	...
	public void deprecatedList() {
		result.use(Results.status())
				.movedPermanentlyTo(ErrorReportController.class).list();
	}
	...
	public void deprecatedAddReport() {
		result.use(Results.status())
				.movedPermanentlyTo(ErrorReportController.class).addReport();
	}
		...
		try {
			if (text != null && text.length() >= 0) {
				...
				if (!loggedUser.isLogged()) {
					...
					if (LOG.isDebugEnabled()) {
						LOG.debug("Saving error report text before login: "
								+ text);
					}
				...
				if (LOG.isDebugEnabled()) {
					...
					for (ProcessResult processResult : pr) {
						LOG.debug("... "
								+ processResult.getTextAnnotatedWithErrors());
					}
				}
				result.include("analyzed", true)
						.include("cleanText", text)
						.include("annotatedText",
								cogrooFacade.getAnnotatedText(text, pr))
						.include("singleGrammarErrorList",
								cogrooFacade.asSingleGrammarErrorList(text, pr))
						.include(
								"omissionCategoriesList",
								this.errorEntryLogic
	...
	@Path("/reports")
	@LoggedIn
	public void addReport(String text, List<String> badint,
			List<String> comments, List<String> badintStart,
			List<String> badintEnd, List<String> badintRule,
	...
	public void veryDeprecatedDetails(ErrorEntry errorEntry) {
		result.use(Results.status())
				.movedPermanentlyTo(ErrorReportController.class)
				.details(errorEntry);
	}
	...
	public void details(ErrorEntry errorEntry) {
		...
		ErrorEntry errorEntryFromDB = errorEntryDAO.retrieve(new Long(
				errorEntry.getId()));
		...
		result.include("errorEntry", errorEntryFromDB)
				.include(
						"processResultList",
						cogrooFacade.cachedProcessText(errorEntryFromDB
								.getText()))
		...
		String version = "Versão: "
				+ errorEntryFromDB.getVersion().getVersion();
		String creationDate = "Criado em: "
				+ DateFormat.getDateTimeInstance(DateFormat.LONG,
						DateFormat.LONG).format(errorEntryFromDB.getCreation());
		String changeDate = "Modificado em: "
				+ DateFormat.getDateTimeInstance(DateFormat.LONG,
						DateFormat.LONG).format(errorEntryFromDB.getModified());
		String type = "Tipo: ";
		String details = null;
	...
	public void editDetails(ErrorEntry errorEntry) {
		...
		if (errorEntryFromDB == null) {
			...
			validator.onErrorUse(Results.logic())
					.redirectTo(ErrorReportController.class).list();
		} else if (!loggedUser.isLogged()
				|| !loggedUser.getUser().getRole().getCanEditErrorReport()) {
			validator.add(new ValidationMessage(
			...
			List<ProcessResult> procRes = cogrooFacade
					.processText(errorEntryFromDB.getText());
			...
			result.include("errorEntry", errorEntryFromDB)
					...
					.include(
							...
							cogrooFacade.asSingleGrammarErrorList(
									errorEntryFromDB.getText(), procRes))
					.include("omissionCategoriesList",
							this.errorEntryLogic.getErrorCategoriesForUser());
		}
	}
	...
	@LoggedIn
	public void updateErrorReport(Long reportId, String type,
			String changedText, String badintIndex, String badintType,
			String badIntMan, List<String> badintStart, List<String> badintEnd,
			List<String> badintRule, String omissionCategory,
			String omissionCustom, String omissionReplaceBy,
			String omissionStart, String omissionEnd) {
		...
		if (changedText != null
				&& !changedText.equals(errorEntryFromDB.getText())) {
		...
		if (type.equals("BADINT")) {
			...
			newBadIntervention.setClassification(Enum.valueOf(
					BadInterventionClassification.class, badintType));
			...
			if (!badIntMan.isEmpty()) {
				...
				newBadIntervention.setRule(badintRule.get(Integer
						.valueOf(badintIndex) - 1));
				errorEntryFromDB.setSpanStart(Integer.valueOf(badintStart
						.get(Integer.valueOf(badintIndex) - 1)));
				errorEntryFromDB.setSpanEnd(Integer.valueOf(badintEnd
						.get(Integer.valueOf(badintIndex) - 1)));
			}

			this.errorEntryLogic.updateBadIntervention(errorEntryFromDB,
					originalErrorEntry);
		} else {
			int start = -1;
			int end = -1;
			...
			if ((omissionReplaceBy == null || omissionReplaceBy.length() == 0)) {
				validator
						.add(new ValidationMessage(
								ExceptionMessages.ERROR_REPORT_OMISSION_MISSING_REPLACE,
								ExceptionMessages.ERROR));
			}
			...
			this.errorEntryLogic.updateOmission(errorEntryFromDB,
					originalErrorEntry);
		}
	...
	public void remove(ErrorEntry errorEntry) {
		...
		if (loggedUser.getUser().equals(errorEntry.getSubmitter())
				|| loggedUser.getUser().getRole()
						.getCanDeleteOtherUserErrorReport()) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("errorEntry: " + errorEntry);
			}
			errorEntryLogic.remove(errorEntry);
		} else {
			LOG.info("Unauthorized user tried to delete errorEntry: "
					+ loggedUser.getUser() + " : " + errorEntry);
		}
	...
	/**
	 * Submit an error report via OpenOffice plugin. Only CoGrOO users are
	 * allowed.
	 * 
	 * @param username
	 * @param token
	...
	public void submitErrorEntry(String username, String token, String error) {
		...
		LOG.debug("Got new error report from: " + username
				+ " encrypted token: " + token + " error: " + error);
		try {
	...
	public void getErrorCategoriesForUser(String username, String token) {
		...
		try {
			...
			Set<String> cat = this.errorEntryLogic
					.getErrorCategoriesForUser(username);
			...
			result.include("result",
					RestUtil.prepareResponse("categories", sb.toString()));
		} catch (Exception e) {
	...
	public void addComment(ErrorEntry errorEntry, String newComment) {
		...
		if (newComment.trim().isEmpty()) {
			validator.add(new ValidationMessage(
					ExceptionMessages.COMMENT_SHOULD_NOT_BE_EMPTY,
					ExceptionMessages.ERROR));
			validator.onErrorUse(Results.logic())
					.redirectTo(ErrorReportController.class)
					.details(errorEntryFromDB);
			...
			validator.add(new ValidationMessage(
					ExceptionMessages.COMMENT_SHOULD_NOT_EXCEED_CHAR,
					ExceptionMessages.ERROR));
			validator.onErrorUse(Results.logic())
					.redirectTo(ErrorReportController.class)
					.details(errorEntryFromDB);
		} else {
			errorEntryLogic.addCommentToErrorEntry(errorEntryFromDB.getId(),
					loggedUser.getUser().getId(), newComment, true);

		}

		result.include("gaEventCommentAdded", true);

		result.redirectTo(ErrorReportController.class)
				.details(errorEntryFromDB);
	}
	...
	public void remove(Comment comment) {
		...
		if ((loggedUser.getUser().getRole().getCanDeleteOwnCommment() && loggedUser
				.getUser().equals(comment.getUser()))
				|| loggedUser.getUser().getRole()
						.getCanDeleteOtherUserCommment()) {
			errorEntryLogic.removeComment(comment);
		} else {
			LOG.warn("Invalid user tried to delete comment "
					+ loggedUser.getUser() + " : " + comment);
		}
	...
	@LoggedIn
	public void addAnswerToComment(ErrorEntry errorEntry, Comment comment,
			String answer) {
		answer = sanitizer.sanitize(answer, true);
		errorEntryLogic.addAnswerToComment(comment.getId(), loggedUser
				.getUser().getId(), answer);
		result.include("gaEventCommentAdded", true);
	...
	public void remove(Comment answer, Comment comment) {
		...
		if ((loggedUser.getUser().getRole().getCanDeleteOwnCommment() && loggedUser
				.getUser().equals(answer.getUser()))
				|| loggedUser.getUser().getRole()
						.getCanDeleteOtherUserCommment()) {
			errorEntryLogic.removeAnswer(answer, comment);
		} else {
			LOG.warn("Invalid user tried to delete answer "
					+ loggedUser.getUser() + " : " + comment);
		}
	}
	...
	public void errorEntrySetPriority(ErrorEntry errorEntry, String priority) {
		if (loggedUser.getUser().getRole().getCanSetErrorReportPriority()) {
			errorEntryLogic.setPriority(errorEntry,
					Enum.valueOf(Priority.class, priority));
	...
	public void errorEntrySetState(ErrorEntry errorEntry, String state) {
		if (loggedUser.getUser().getRole().getCanSetErrorReportState()) {
			errorEntryLogic.setState(errorEntry,
