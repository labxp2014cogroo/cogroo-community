...
/**
 * Access point to CoGrOO. Each session will instantiate one
 * {@link CogrooFacade}, so each user will have one.
 */
@Component
@ApplicationScoped
public class CogrooFacade {
		...
		if (theCogroo == null) {
			synchronized (this) {
				if (theCogroo == null) {
					...
					try {
						this.theCogroo = new GrammarChecker(
								factory.createPipe());
					} catch (Exception e) {
					...
					this.ruleDefinitionList = ((GrammarChecker) theCogroo)
							.getRuleDefinitions();

					if (LOG.isDebugEnabled()) {
						LOG.debug("Loaded " + this.ruleDefinitionList.size()
								+ " rule definitions");
					}
	...
	// TODO: if we return detailed information we can handle it in JSP and add
	// colors
	/**
		...
		try {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Got " + document.getSentences().size()
						+ " sentences with a total of "
						+ document.getMistakes().size() + " mistakes.");
			}
			for (Sentence sentence : document.getSentences()) {
				List<Mistake> filteredMistakes = filterMistakes(sentence,
						document.getMistakes());

				ProcessResult pr = new ProcessResult();
				pr.setSyntaxTree(sentence.asTree().toSyntaxTree());
				pr.setTextAnnotatedWithErrors(annotateText(sentence,
						filteredMistakes));
	...
	private static String memory() {
		...
		return String.format("Free: %.2f; Max: %.2f; Total: %.2f", free, max,
				total);
	...
	public String getAnnotatedText(String text,
		...
		Span[] spans = sortedMistakes.keySet().toArray(
				new Span[sortedMistakes.size()]);
		for (int i = spans.length - 1; i >= 0; i--) {
			sb.insert(spans[i].getEnd(), "</span>");
			sb.insert(spans[i].getStart(),
					"<span class=\"grammarerror\" title=\""
							+ sortedMistakes.get(spans[i]).getShortMessage()
							+ "\">");
		}
		if (LOG.isDebugEnabled()) {
	...
	public List<SingleGrammarError> asSingleGrammarErrorList(String text,
			List<ProcessResult> processResult) {
		if (LOG.isDebugEnabled()) {
		...
		for (int i = 0; i < spans.length; i++) {
			...
			singleGEList.add(new SingleGrammarError(sb.toString(),
					sortedMistakes.get(spans[i])));
		}
		if (LOG.isDebugEnabled()) {
	...
	private String annotateText(Sentence sentence,
			List<Mistake> filteredMistakes) {
		SortedMap<Span, Mistake> sortedMistakes = new TreeMap<Span, Mistake>();
		...
		for (int i = spans.length - 1; i >= 0; i--) {
			...
			text.insert(spans[i].getStart() - sentence.getStart(),
					"<span class=\"grammarerror\" title=\""
							+ sortedMistakes.get(spans[i]).getShortMessage()
							+ "\">");
		}
	...
	private List<Mistake> filterMistakes(Sentence sentence,
