...
public class UserDAO {
	...
	public long count() {
		return (Long) em.createQuery("SELECT count(*) from " + USER_ENTITY)
				.getSingleResult();
	}
	...
	private User retrieveSingle(String value1, String field1, String value2,
			String field2) {
		User user = null;
		try {
			user = (User) em
					.createQuery(
							"from " + USER_ENTITY + " w where w." + field1
									+ "=?1 and w." + field2 + "=?2")
					.setParameter(1, value1).setParameter(2, value2)
					.getSingleResult();
	...
	@SuppressWarnings("unchecked")
	private List<User> retrieveAll(String value1, String field1, String value2,
			String field2) {
