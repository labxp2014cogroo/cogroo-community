...
public class InterceptorUtil {
	...
	public static void intercept(Method method, LoggedUser loggedUser,
			Result result) {
		...
		if (loggedInInterceptor.accepts(resourceMethod)) {
			loggedInInterceptor.intercept(stack, resourceMethod,
					resourceInstance);
		}

		RoleNeededInterceptor roleNeededInterceptor = new RoleNeededInterceptor(
				result, loggedUser);

		if (roleNeededInterceptor.accepts(resourceMethod)) {
			roleNeededInterceptor.intercept(stack, resourceMethod,
					resourceInstance);
		}
	}

	public static void validate(Method method, LoggedUser loggedUser,
			Result result) {
		intercept(method, loggedUser, result);
		if (result.included().containsKey("errors")) {
			@SuppressWarnings("unchecked")
			List<Message> list = (List<Message>) result.included()
					.get("errors");
